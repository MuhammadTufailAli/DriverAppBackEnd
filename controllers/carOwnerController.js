...
const carOwnerLogin = async function (req, res) {
  ...
  try {
    ...
    if (
      ...
      res.send({
        code: 0,
        message: 'Car Owner login successful',
        data: response,
      });
      ...
      res.send({
        code: -3,
        error: 'Car Owner account yet needs an appproval of Admin',
      });
    } else {
  ...
  try {
    ...
    if (allreadyBooked.length > 0) {
      ...
      res.send({
        code: 1,
        message:
          'You have Already Hired a driver.You cannot hire more than one driver at a time ',
        data: [],
      });
    } else {
      const response = await driver.find({
        status: { $nin: ['new', 'blocked', 'booked'] },
      });
      console.log('Car Owner retrive response ', response);
      res.send({ code: 0, data: response });
    }
  ...
  try {
    ...
    if (!matchingRequest) {
      res.send({
        code: 1,
        data: [],
        message: 'You have not hired any Driver Yet',
      });
    } else {
...
const driverRequestView = async (req, res) => {
  ...
  try {
    const response = await driverAttendance.find({
      reqId: reqId,
      status: 'new',
    });
    ...
    if (response && response.length > 0) {
      ...
      res.send({
        code: 1,
        data: response,
        message: 'No new attendace requests',
      });
    }
  } catch (e) {
    console.log('Error occured, possible cause: ' + e.message);
