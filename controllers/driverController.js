...
const getexpenses = async (req, res) => {
  ...
  try {
    let response = await DriverExpense.find({ userName: name });
...
const addexpenses = async (req, res) => {
  try {
    ...
    let response = await DriverExpense.create(req.body);
    console.log(response);
    res.status(201).json({
      status: 'sucess',
      data: {
...
const driverLogin = async function (req, res) {
  ...
  try {
    ...
    if (
      ...
      res.send({
        code: -3,
        error: 'Driver account yet needs an appproval of Admin',
      });
    } else {
...
const driverViewAllReq = async (req, res) => {
  ...
  try {
    const allreadyBooked = await OwnerRequest.find({
    ...
    if (allreadyBooked.length > 0) {
      ...
      res.send({
        code: 1,
        message: 'You have Already Hired by a carOwner',
        data: [],
      });
    } else {
      const response = await OwnerRequest.find({
        driverUserName: driverUserName,
        status: 'new',
      });
      console.log('Car Owner retrive response ', response);
  ...
  if (reqStatus === 'accepted') {
    try {
      ...
      res.send({
        ...
        driverResponse: driverBooked,
        requestResponse: reqResponse,
        carOwnerResponse: carOwner,
      });
    } catch (e) {
      console.log('Error occured, possible cause: ' + e.message);
      res.send({
...
const hireBY = async (req, res) => {
  ...
  try {
    const matchingRequest = await OwnerRequest.findOne({
      driverUserName: driverUserName,
      status: { $nin: ['new', 'expired'] },
    });
    if (!matchingRequest) {
      res.send({
        code: 1,
        data: [],
        message: 'You have not hired by any One Yet',
      });
    } else {
...
const attendanceRequest = async (req, res) => {
  ...
  try {
    let exist = await driverAttendance.findOne({
      reqId: reqId,
      createdDate: { $gte: new Date(today), $lt: new Date(tomorrow) },
    });
...
const driverAttendanceHistory = async (req, res) => {
  ...
  try {
    const response = await driverAttendance
      .find({ reqId: reqId, status: 'accepted' })
      .sort({ createdDate: -1 });
    const count = await driverAttendance
      .find({ reqId: reqId, status: 'accepted' })
      .count();
    const matchingRequest = await OwnerRequest.findOne({
      _id: reqId,
...
const addExpense = async (req, res) => {
  ...
  try {
    ...
    res.send({
      code: 0,
      data: response,
      message: 'Infomation added successfully',
    });
  } catch (e) {
    console.log('Error occured, possible cause: ' + e.message);
    res.send({
